...
R = S + 12;
assert(norm(R(0.3,0.5) - S(0.3,0.5) - 12) < 1e-12); 
R = 12 + S;
assert(norm(R(0.3,0.5) - S(0.3,0.5) - 12) < 1e-12); 
...
s2 = Basis.Function(b2,c2);
p = s1*s2;
assert(norm(p(1.5)-s1(1.5)*s2(1.5)) < 1e-12);
p = s2*s1;
assert(norm(p(1.5)-s2(1.5)*s1(1.5)) < 1e-12);
...
assert(norm(delta(:)) < 1e-14); 
P = S2*S1;
delta = abs(P(0.3,0.5) - S2(0.3,0.5)*S1(0.3,0.5));
assert(norm(delta(:)) < 1e-14); 
