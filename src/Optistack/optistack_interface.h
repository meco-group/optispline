...
class OptiSplineInterface {
  public:
    ...
    spline::Function Function(const spline::TensorBasis& b,
        const std::vector<casadi_int>& shape=std::vector<casadi_int>(),
      ...
      casadi_int n = shape.size()>=1 ? shape[0] : 1;
      casadi_int m = shape.size()==2 ? shape[1] : 1;
  ...
  private:
    #ifndef SWIG
      typedef std::function<MX(T&, casadi_int, casadi_int, const std::string&)> Opti_create;
      MT coeff_varpar(Opti_create& opti_create, const std::vector<casadi_int>& shape, casadi_int n=1, casadi_int m=1, const std::string& attribute="full") {
        casadi_int cs = casadi::product(shape);

        if (attribute=="full") {
          std::vector<casadi_int> ret_shape = shape;
          ret_shape.push_back(n);
          ret_shape.push_back(m);
          return MT(opti_create(static_cast<T &>(*this), casadi::product(ret_shape), 1, "full"), ret_shape);
        }

        std::vector<MX> args;
        for (casadi_int i=0;i<cs;++i)
          args.push_back(opti_create(static_cast<T &>(*this), n, m, attribute));

        std::vector<casadi_int> t_shape = {n, m};
        t_shape.insert(t_shape.end(), shape.begin(), shape.end());
        MT t = MT(horzcat(args), t_shape);

        std::vector<casadi_int> reorder = casadi::range(2, shape.size()+2);
        reorder.push_back(0);
        reorder.push_back(1);

        return t.reorder_dims(reorder);
      }
      MT coeff_variable(const std::vector<casadi_int>& shape, casadi_int n=1, casadi_int m=1, const std::string& attribute="full") {
        Opti_create opti_creator = [](T& opti, casadi_int nn, casadi_int mm, const std::string& aattribute) { return opti.variable(nn, mm, aattribute); };
        return coeff_varpar(opti_creator, shape, n, m, attribute);
      }
      MT coeff_parameter(const std::vector<casadi_int>& shape, casadi_int n=1, casadi_int m=1, const std::string& attribute="full") {
        Opti_create opti_creator = [](T& opti, casadi_int nn, casadi_int mm, const std::string& aattribute) { return opti.parameter(nn, mm, aattribute); };
        return coeff_varpar(opti_creator, shape, n, m, attribute);
      }
    #endif
};
